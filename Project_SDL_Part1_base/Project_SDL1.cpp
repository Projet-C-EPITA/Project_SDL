...
unsigned ground::update()
{
    ...
    for (auto &animal_ptr : animals)
    {
        ...
        if (animal_ptr->type == WOLF)
        {
            ...
            wolfs->get_neareast_animal(
                ground::animals); // TODO :va en meme temps calculer closest dog
        }
    ...
    if (total_babies > 1)
    {
        ...
        if (nb_babies == 1)
        {
            ...
            auto baby =
                std::make_shared<sheep>(file_path_sheep, window_surface_ptr_);
            add_animal(baby);
            baby->type = SHEEP;
            int random = rand() % 2;
            if (random == 0)
...
application::application(unsigned n_sheep, unsigned n_wolf)
{
    ...
    {
        std::shared_ptr<wolf> wolfs =
            std::make_shared<wolf>(file_path_wolf, window_surface_ptr_);
        ground_->add_animal(wolfs);
        wolfs->draw();
        wolfs->type = WOLF;
    }
    std::shared_ptr<shepherd> shepherd1 =
        std::make_shared<shepherd>(file_path_shepherd, window_surface_ptr_);
...
{
    window_ptr_ = SDL_CreateWindow("Game SDL", SDL_WINDOWPOS_CENTERED,
    ...
    window_renderer_ =
        SDL_CreateRenderer(window_ptr_, -1, SDL_RENDERER_ACCELERATED);
...
int application::loop(unsigned period)
{
    ...
    SDL_DestroyWindow(window_ptr_);
    SDL_Quit();
    std::cout << "The score of the application is: " << ground_->scoreF
              << std::endl;
    return period;
}
